import os
# Ensure MODEL_BASE is set before importing hyvideo.constants, otherwise constants
# will fallback to the default relative path (./weight/HunyuanVideo_I2V) at import time.
# MovieAgent passes an absolute path via args.model_base.
...
class Inference(object):
    ...
    def load_state_dict(args, model, pretrained_model_path):
        ...
        # Lower peak host-memory during checkpoint load:
        # - default map_location=auto: if model lives on CUDA and CPU offload is disabled, load directly to that CUDA device.
        # - fallback to CPU if anything fails.
        load_map = os.getenv("HUNYUAN_TORCH_LOAD_MAP", "auto").strip().lower()
        model_device = next(model.parameters()).device
        if load_map in ["cpu", "host"]:
            map_location = torch.device("cpu")
        elif load_map in ["cuda", "gpu"]:
            map_location = model_device if model_device.type == "cuda" else torch.device("cuda")
        else:
            # auto
            if (not getattr(args, "use_cpu_offload", False)) and model_device.type == "cuda":
        ...
        try:
            ...
            if "mmap" in sig.parameters:
                use_mmap = os.getenv("HUNYUAN_TORCH_LOAD_MMAP", "1") not in ("0", "false", "False", "")
                load_kwargs["mmap"] = use_mmap
            if "weights_only" in sig.parameters:
                use_weights_only = os.getenv("HUNYUAN_TORCH_LOAD_WEIGHTS_ONLY", "0") not in ("0", "false", "False", "")
                load_kwargs["weights_only"] = use_weights_only
        except Exception:
            pass

        logger.info(f"torch.load map_location={map_location}, kwargs={load_kwargs}")
        try:
            state_dict = torch.load(model_path, map_location=map_location, **load_kwargs)
        except TypeError:
            # Compatibility fallback for older torch versions / custom checkpoints
            state_dict = torch.load(model_path, map_location=map_location)
        except Exception:
            # Last-resort fallback to CPU loading
            logger.warning("torch.load with selected map_location failed; fallback to CPU loading")
        ...
        **kwargs,
        ...
        Args:
            ...
            kwargs:
                ...
                infer_steps (int): The number of inference steps. Default is 70.
                guidance_scale (float): The guidance scale for the generation. Default is 7.0.
        ...
        if i2v_mode:
            ...
            ref_image_transform = transforms.Compose([
                transforms.Resize(closest_size, interpolation=InterpolationMode.BICUBIC, antialias=True),
