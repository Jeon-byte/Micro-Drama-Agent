...
class HunyuanVideoPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            uncond_input = text_encoder.text2tokens(uncond_tokens, data_type=data_type, semantic_images=uncond_image)
    ...
    def prepare_latents(
        ...
        if i2v_mode and i2v_stability:
            ...
            # Higher t => more noise, lower first-frame adherence. Keep default at 0.999
            # for compatibility, but expose env override for stronger reference locking.
            try:
                stability_t = float(os.environ.get("HUNYUAN_I2V_STABILITY_T", "0.999"))
            except Exception:
                stability_t = 0.999
            stability_t = max(0.0, min(1.0, stability_t))
            t = torch.tensor([stability_t], device=device, dtype=dtype)
            latents = x0 * t + x1 * (1.0 - t)
