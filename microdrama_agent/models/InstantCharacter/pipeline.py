...
class InstantCharacterFluxPipeline(FluxPipeline):
    ...
    def encode_siglip_image_emb(self, siglip_image, device, dtype):
        # If encoders are kept on CPU, ensure inputs match encoder device/dtype.
        enc = getattr(self, "siglip_image_encoder", None)
        if enc is not None:
            try:
                ...
                enc_dtype = dtype
            siglip_image = siglip_image.to(enc_device, dtype=enc_dtype)
        else:
        ...
        # Move embeds to requested device/dtype for downstream projector/attention.
        siglip_image_embeds = siglip_image_embeds.to(device, dtype=dtype)
        siglip_image_shallow_embeds = siglip_image_shallow_embeds.to(device, dtype=dtype)
        return siglip_image_embeds, siglip_image_shallow_embeds


    @torch.inference_mode()
    def encode_dinov2_image_emb(self, dinov2_image, device, dtype):
        enc = getattr(self, "dino_image_encoder_2", None)
        if enc is not None:
            try:
                p = next(enc.parameters())
                enc_device = p.device
                ...
                enc_device = torch.device("cpu")
                enc_dtype = dtype
            dinov2_image = dinov2_image.to(enc_device, dtype=enc_dtype)
        else:
        ...
        dinov2_image_embeds = dinov2_image_embeds.to(device, dtype=dtype)
        dinov2_image_shallow_embeds = dinov2_image_shallow_embeds.to(device, dtype=dtype)
    ...
    def init_ccp_and_attn_processor(self, *args, **kwargs):
        ...
        # BUGFIX: device was mistakenly read from `nb_token`.
        ...
        # Optional: keep projector on CPU to reduce peak GPU memory during init.
        # (Main generation still needs the transformer on GPU; projector is used to build ip_hidden_states only.)
        keep_proj_cpu = os.getenv("INSTANTCHAR_PROJECTOR_CPU", "0") not in ("0", "false", "False")
        if keep_proj_cpu:
            image_proj_model.to(torch.device("cpu"), dtype=dtype)
        else:
    ...
    def init_adapter(
        ...
        # NEW: optionally keep encoders on CPU to reduce VRAM
        keep_enc_cpu = os.getenv("INSTANTCHAR_ENCODER_CPU", "1") not in ("0", "false", "False")
        enc_device = torch.device("cpu") if keep_enc_cpu else device
        ...
        image_encoder.eval()
        image_encoder.to(enc_device, dtype=dtype)
        ...
        image_encoder_2.to(enc_device, dtype=dtype)
    ...
    def _infer_img_hw_from_latents(self, latents: torch.FloatTensor, height: int, width: int):
        """Infer image token grid (h,w) for bbox routing.

        We prefer to use the known VAE scale factor; if it doesn't match latents sequence length, fallback to square-ish.
        """
        try:
            h = int(height // self.vae_scale_factor)
            w = int(width // self.vae_scale_factor)
            if h * w == int(latents.shape[1]):
                return (h, w)
        except Exception:
            pass

        # fallback: best-effort factorization (prefer w>=h)
        seq = int(latents.shape[1])
        import math
        h = int(math.floor(math.sqrt(seq)))
        while h > 1 and seq % h != 0:
            h -= 1
        w = seq // h if h > 0 else seq
        if w < h:
            h, w = w, h
        return (int(h), int(w))

    def _bbox_to_latent_bbox(self, bbox, img_hw, height: int, width: int):
        """Convert pixel/normalized bbox to latent-grid bbox (x0,y0,x1,y1)."""
        if bbox is None:
            return None
        try:
            x0, y0, x1, y1 = bbox[:4]
            ...
            # normalized bbox
            if mx <= 1.5:
                x0, x1 = float(x0) * float(width), float(x1) * float(width)
                y0, y1 = float(y0) * float(height), float(y1) * float(height)
            else:
                x0, y0, x1, y1 = float(x0), float(y0), float(x1), float(y1)

            h, w = img_hw
            sx = float(w) / float(width)
            sy = float(h) / float(height)
            lx0 = max(0, min(w, int(x0 * sx)))
            lx1 = max(0, min(w, int(x1 * sx + 0.9999)))
            ly0 = max(0, min(h, int(y0 * sy)))
            ly1 = max(0, min(h, int(y1 * sy + 0.9999)))
    ...
    def __call__(
        ...
        subject_emb_dicts: Optional[List[Dict[str, Any]]] = None,
        ...
        # Pre-compute image token grid for bbox routing
        img_hw = self._infer_img_hw_from_latents(latents, height, width)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # one-time debug print for multi-subject routing
                if i == 0:
                    try:
                        if subject_emb_dicts is None:
                            print("[InstantCharacter][DEBUG][pipeline] subject_emb_dicts=None")
                        else:
                            roles = []
                            none_bbox = []
                            for it in subject_emb_dicts:
                                if not isinstance(it, dict):
                                    continue
                                roles.append(str(it.get("role", "")))
                                # bbox_latent computed later; here only tell whether bbox exists
                                if it.get("bbox", None) is None:
                                    none_bbox.append(str(it.get("role", "")))
                            print(
                                f"[InstantCharacter][DEBUG][pipeline] subject_emb_dicts_len={len(subject_emb_dicts)} roles={roles} roles_missing_bbox={none_bbox}"
                            )
                    except Exception as e:
                        print(f"[InstantCharacter][DEBUG][pipeline] debug_print_failed: {e}")
                ...
                # subject adapter (single)
                if subject_image_embeds_dict is not None:
                ...
                # subject adapter (multi)
                if subject_emb_dicts is not None and isinstance(subject_emb_dicts, list) and len(subject_emb_dicts) > 0:
                    routed = []
                    for it in subject_emb_dicts:
                        if not isinstance(it, dict):
                            continue
                        ip_hid = it.get('ip_hidden_states', None)
                        if ip_hid is None:
                            continue
                        bbox_latent = self._bbox_to_latent_bbox(it.get('bbox', None), img_hw, height, width)
                        routed.append(
                            dict(
                                role=it.get('role', ''),
                                ip_hidden_states=ip_hid,
                                scale=float(it.get('scale', subject_scale)),
                                bbox_latent=bbox_latent,
                            )
                        )
                    if routed:
                        # debug: report bbox_latent None status once
                        if i == 0:
                            try:
                                brief = [(str(x.get('role','')), x.get('bbox_latent', None) is None) for x in routed]
                                print(f"[InstantCharacter][DEBUG][pipeline] routed_roles_bbox_latent_is_none={brief}")
                            except Exception as e:
                                print(f"[InstantCharacter][DEBUG][pipeline] routed_debug_failed: {e}")

                        self._joint_attention_kwargs['emb_dict'] = dict(
                            length_encoder_hidden_states=prompt_embeds.shape[1],
                            img_hw=img_hw,
                        )
                        self._joint_attention_kwargs['subject_emb_dicts'] = routed
    ...
    def enable_sequential_cpu_offload(self, gpu_id: Optional[int] = None, device: Union[torch.device, str] = "cuda"):
        ...
        Arguments:
            ...
            device (`torch.Device` or `str`, *optional*, defaults to "cuda"):
                ...
                default to "cuda".
        """
        # Newer diffusers has this guard; older versions (e.g. 0.32.x) may not.
        _maybe = getattr(self, "_maybe_raise_error_if_group_offload", None)
        if callable(_maybe):
            _maybe(raise_error=True)
